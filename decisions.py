...
class decision_maker(Node):
    ...
    def __init__(self, publisher_msg, publishing_topic, qos_publisher, goalPoint, rate=10, motion_type=POINT_PLANNER):
        ...
        if motion_type == POINT_PLANNER:
            self.controller=controller(klp=0.4, klv=0.3, kli=0.07, kap=1.1, kav=0.5, kai=0.0)
            self.planner=planner(POINT_PLANNER)    
    
    
        elif motion_type==TRAJECTORY_PLANNER:
            self.controller=trajectoryController(klp=0.5, klv=0.25, kli=0.05, kap=1.2, kav=0.6, kai=0.0)
...
def main(args=None):
    ...
    if args.motion.lower() == "point":
        DM = decision_maker(Twist, "/cmd_vel", odom_qos, goal_point, rate=10, motion_type=POINT_PLANNER)
    elif args.motion.lower() == "trajectory":
        trajectory_points = [[0.3, 0.0], [0.7, 0.0], [1.0, 0.3]]
        DM = decision_maker(Twist, "/cmd_vel", odom_qos, trajectory_points, rate=10, motion_type=TRAJECTORY_PLANNER)
    else:
